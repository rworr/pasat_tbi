...
class EnsembleArray(nengo.Network):
    ...
    def __init__(self, n_neurons, n_ensembles, ens_dimensions=1,
                 neuron_nodes=False, transforms=None,
        ...
        with self:
            ...
            for i in range(n_ensembles):
                ...
                if self.transforms is None:
                    ...
                    nengo.Connection(self.input[i * ens_dimensions:
                                                (i + 1) * ens_dimensions],
                                     e, synapse=None, transform=self.transforms.next())
        ...
        for i, ens in enumerate(self.ea_ensembles):
            if self.transforms is None:
                ...
                nengo.Connection(
                    self.neuron_input[i * self.n_neurons_per_ensemble:
                                      (i + 1) * self.n_neurons_per_ensemble],
                    ens.neurons, synapse=None, transform=self.transforms.next())
    ...
    def add_output(self, name, function, synapse=None, **conn_kwargs):
        ...
        for i, e in enumerate(self.ea_ensembles):
            if self.transforms is None:
                ...
                nengo.Connection(
                    e.neurons, output[indices[i]:indices[i+1]], transform=self.transforms.next(),
                    synapse=synapse, **conn_kwargs)
