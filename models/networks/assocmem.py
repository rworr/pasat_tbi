...
class AssociativeMemory(nengo.Network):
    ...
    def __init__(self, input_vectors, output_vectors=None,  # noqa: C901
                 ...
                 inhibitable=False, transforms=None,
        ...
        with self, self.am_ens_config:
            ...
            for i in range(self.n_items):
                ...
                if self.transforms is None:
                    ...
                    nengo.Connection(self.bias_node, e, transform=self.transforms.next())
                    nengo.Connection(self.elem_input[i], e, transform=self.transforms.next())
                    nengo.Connection(
                        e.neurons, self.elem_output[i], transform=self.transforms.next())
            ...
            if inhibitable:
                ...
                if self.transforms is None:
                    ...
                    nengo.Connection(self.inhibit, self.elem_input,
                                     transform=self.transforms.next())
    ...
    def add_input_mapping(self, name, input_vectors, input_scales=1.0):
        ...
        if self.transforms is None:
            ...
            nengo.Connection(in_node, self.elem_input,
                             synapse=None, transform=self.transforms.next())
        ...
        with self.default_ens_config:
            ...
            if self.thresh_ens is not None:
                if self.transforms is None:
                    ...
                    c = nengo.Connection(self.thresh_ens.output, default_vector_ens,
                                         transform=self.transforms.next())
            else:
                if self.transforms is None:
                    c = nengo.Connection(
            ...
            if self.inhibit is not None:
                if self.transforms is None:
                    ...
                    nengo.Connection(self.inhibit, default_vector_ens,
                                     transform=self.transforms.next(), synapse=None)
    ...
    def add_threshold_to_outputs(self, n_neurons=50, inhibit_scale=10):
        ...
        if self.transforms is not None:
            warnings.warn("Transforms are not passed through for thresholded output")
